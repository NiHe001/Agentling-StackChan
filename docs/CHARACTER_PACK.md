...
`loop: false` 的行为在最后一步后保留约 850 ms，然后固件根据最新任务汇总状态恢复表情；
对于工作、空闲和等待等持续状态，还会恢复对应行为的灯光。因此完成庆祝或额度告警不会长期
覆盖另一个正在工作的任务。
`loop: true` 适合工作、空闲和等待批准等持续状态，会在状态变化或新行为到来时中断。

## 动作安全

角度单位是 0.1 度；`yaw` 范围 -900..900，`pitch` 范围 -450..450，`speed` 范围
1..1000。桌面端和固件都会再次限幅。固件每次动作最多保持扭矩 3 秒，随后自动释放；角色包
不能绕过这个上限。当前固件不会自动执行行为时间线里的 `motion`，只有明确的硬件命令
会触发舵机动作。
